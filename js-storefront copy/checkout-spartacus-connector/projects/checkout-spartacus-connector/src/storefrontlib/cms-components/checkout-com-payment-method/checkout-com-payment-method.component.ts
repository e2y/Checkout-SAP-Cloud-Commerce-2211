...
export class CheckoutComPaymentMethodComponent extends CheckoutPaymentMethodComponent implements OnInit {
  ...
  public isFlowEnabled$: Observable<boolean> = of(false);
  ...
  initializeFlow(): void {
    this.checkoutPaymentFacade.initializeFlow().pipe(
      takeUntilDestroyed(this.destroyRef)
    ).subscribe();
    this.isLoading$ = this.checkoutPaymentFacade.getIsProcessing();
    this.isFlowEnabled$ = this.checkoutPaymentFacade.getIsFlowEnabled();

    this.isFlowEnabled$.pipe(
      take(1),
      takeUntilDestroyed(this.destroyRef)
    ).subscribe({
