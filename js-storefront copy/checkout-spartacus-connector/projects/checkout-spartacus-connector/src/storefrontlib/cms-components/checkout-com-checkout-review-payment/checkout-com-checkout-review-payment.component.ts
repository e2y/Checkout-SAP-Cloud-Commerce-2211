...
export class CheckoutComCheckoutReviewPaymentComponent implements OnInit {
  ...
  paymentDetails$: Observable<CheckoutComPaymentDetails | undefined>;
  ...
  ngOnInit(): void {
    this.paymentDetails$ = this.checkoutPaymentFacade.getIsFlowEnabled().pipe(
      switchMap((isFlowEnabled: boolean): Observable<CheckoutComPaymentDetails> => {
        if (isFlowEnabled === true) {
