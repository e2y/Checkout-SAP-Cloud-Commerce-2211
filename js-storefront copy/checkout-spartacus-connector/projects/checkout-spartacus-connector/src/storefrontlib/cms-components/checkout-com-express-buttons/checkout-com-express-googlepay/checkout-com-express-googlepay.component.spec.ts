...
import { CheckoutComConnector } from '../../../../core/connectors/checkout-com/checkout-com.connector';
import { CheckoutComCheckoutBillingAddressFacade } from '../../../../core/facades/checkout-com-checkout-billing-address.facade';
import { CheckoutComFlowFacade } from '../../../../core/facades/checkout-com-flow.facade';
import { CheckoutComGooglepayFacade } from '../../../../core/facades/checkout-com-googlepay.facade';
import { CheckoutComOrderFacade } from '../../../../core/facades/checkout-com-order.facade';
import { CheckoutComPaymentFacade } from '../../../../core/facades/checkout-com-payment.facade';
import { GooglePayPaymentDataRequest, GooglePayPaymentExpressIntents, GooglePayPaymentRequest } from '../../../../core/model/GooglePay';
import { CheckoutComBillingAddressFormService } from '../../../../core/services/billing-address-form/checkout-com-billing-address-form.service';
...
describe('CheckoutComExpressGooglepayComponent', () => {
  ...
  beforeEach(() => {
    ...
    mockCheckoutComGooglePayFacade.onPaymentAuthorized.and.returnValue(of({}));
  ...
  describe('initExpressPaymentsClient', () => {
    ...
    it('should initialize paymentsClient with provided client settings', async () => {
      ...
      const mockMerchantConfiguration = {
      ...
      const googlePayPaymentDataRequest: GooglePayPaymentDataRequest = {
        apiVersion: 2,
        apiVersionMinor: 0
      };

      const googlepayPaymentExpressIntents: GooglePayPaymentExpressIntents = {
        ...mockMerchantConfiguration,
        callbackIntents: []
      };
      ...
      mockCheckoutComGooglePayFacade.getMerchantConfigurationFromState.and.returnValue(of(mockMerchantConfiguration));
      mockCheckoutComGooglePayFacade.createFullPaymentRequest.and.returnValue(googlePayPaymentDataRequest);
      mockCheckoutComGooglePayFacade.addPaymentExpressIntents.and.returnValue(of(googlepayPaymentExpressIntents));
      ...
      expect(mockCheckoutComGooglePayFacade.createFullPaymentRequest).toHaveBeenCalledWith(mockMerchantConfiguration);
      expect(mockCheckoutComGooglePayFacade.addPaymentExpressIntents).toHaveBeenCalledWith(googlePayPaymentDataRequest);
      expect(mockCheckoutComGooglePayFacade.onPaymentAuthorized).toHaveBeenCalledWith(fullPaymentRequest);
    });
  ...
  describe('authorisePayment', () => {
    ...
    it('should call onPaymentAuthorized when loadPaymentData resolves', fakeAsync(() => {
      ...
      mockCheckoutComGooglePayFacade.addPaymentExpressIntents.and.returnValue(of(mockPaymentDataRequest));
      mockCheckoutComGooglePayFacade.onPaymentAuthorized.and.returnValue(of({}));
    ...
    it('should log error when loadPaymentData rejects', fakeAsync(() => {
      ...
      expect(component['logger'].error).toHaveBeenCalledWith('Error loading payment data:', {err: mockError});
    }));
