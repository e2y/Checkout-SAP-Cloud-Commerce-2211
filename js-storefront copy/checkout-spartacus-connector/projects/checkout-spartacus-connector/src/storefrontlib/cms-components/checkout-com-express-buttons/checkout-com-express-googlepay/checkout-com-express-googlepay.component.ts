...
import { GooglePayMerchantConfiguration, GooglePayPaymentExpressIntents, IntermediatePaymentData } from '../../../../core/model/GooglePay';
...
export class CheckoutComExpressGooglepayComponent extends CheckoutComApmGooglepayComponent implements OnInit, AfterViewInit, OnChanges {
    ...
    this.checkoutComGooglePayFacade.getMerchantConfigurationFromState()
      .pipe(
        ...
        // eslint-disable-next-line  @typescript-eslint/no-explicit-any
          ...
          (paymentData: IntermediatePaymentData): Promise<any> => firstValueFrom(this.checkoutComGooglePayFacade.onPaymentDataChanged(paymentData));
  ...
  protected override authorisePayment(): void {
    ...
    this.checkoutComGooglePayFacade.getMerchantConfigurationFromState().pipe(
      filter((config: GooglePayMerchantConfiguration): boolean => Boolean(config) && Object.keys(config).length > 0),
      distinctUntilChanged(),
      switchMap((config: GooglePayMerchantConfiguration): Observable<GooglePayPaymentExpressIntents> =>
        this.checkoutComGooglePayFacade.addPaymentExpressIntents(this.checkoutComGooglePayFacade.createFullPaymentRequest(config))
      ),
      switchMap((paymentDataRequest: any): Observable<any> => from(this.paymentsClient.loadPaymentData(paymentDataRequest))),
      switchMap((paymentRequest: any): Observable<any> => this.checkoutComGooglePayFacade.onPaymentAuthorized(paymentRequest)),
      ...
      error: (err: unknown): void =>
        this.logger.error('Unable to process express payment. Please try again.', { err })
    });
  }
}
