...
describe('CheckoutComGooglepayService', () => {
  ...
  it('should add payment express intents with shipping address and option required', () => {
    ...
    service.addPaymentExpressIntents(paymentRequest).subscribe({
      next: (result) => {
        expect(result).toEqual({
          ...paymentRequest,
  ...
  it('should add payment express intents with empty shipping address parameters', (done) => {
  ...
  it('should handle payment authorization successfully', (done) => {
    ...
    service.onPaymentAuthorized(paymentData).subscribe({
      next: (result) => {
        expect(result).toEqual({
          googlePayAuth: null,
          googlePayMerchantConfiguration: null,
  ...
  it('should handle payment authorization with error', (done) => {
    const paymentData: GooglePayPaymentRequest = {
      apiVersion: 2,
      apiVersionMinor: 0,
      paymentMethodData: {
    ...
    service.onPaymentAuthorized(paymentData).subscribe({
      next: (session) => {
        expect(session.googlePayPaymentAuthorizationResult).toEqual({ transactionState: 'ERROR' });
        expect(consoleSpy).toHaveBeenCalledWith('Error authorising Google Pay payment', { error });
  ...
  it('should update payment data successfully and resolve promise', async () => {
    ...
    service.onPaymentDataChanged(paymentData).subscribe({
      next: (result) => {
