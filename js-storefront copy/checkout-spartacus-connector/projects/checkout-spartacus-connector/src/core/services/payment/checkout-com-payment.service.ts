...
import { CHECKOUT_COM_ADDRESS_NORMALIZER } from '../../connectors';
...
import { CheckoutComPaymentFacade } from '../../facades/checkout-com-payment.facade';
import { CheckoutComCustomerConfiguration, CheckoutComPaymentDetails } from '../../interfaces';
  ...
  protected setPaymentAddressCommand: Command<Address, Address> = this.commandService.create(
    (address: Address): Observable<Address> => this.checkoutPreconditions().pipe(
      ...
      switchMap(({
        ...
        cartId,
        normalizedAddress
      }: { userId: string, cartId: string, normalizedAddress: Address }): Observable<Address> =>
