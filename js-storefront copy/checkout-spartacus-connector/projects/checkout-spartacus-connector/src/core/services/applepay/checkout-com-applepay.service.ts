...
import { CheckoutComApplepayConnector } from '../../connectors/checkout-com-applepay/checkout-com-applepay.connector';
import { CheckoutComApplePayPaymentRequestEvent } from '../../events/apple-pay.events';
import { CheckoutComApplepayFacade } from '../../facades/checkout-com-applepay.facade';
import {
  ApplePayAuthorization,
  ApplePayPaymentContact,
  ApplePayPaymentRequest,
  ApplepaySession,
  ApplePayShippingContactUpdate, ApplePayShippingMethod,
  ApplePayShippingMethodUpdate
} from '../../model/ApplePay';
import { getUserIdCartId } from '../../shared/get-user-cart-id';
import { createApplePaySession } from './applepay-session';
import { showPaymentMethodFailMessage } from '../../shared/paymentDetails';
...
export class CheckoutComApplepayService extends OrderService implements CheckoutComApplepayFacade {
  ...
  public requestApplePayPaymentRequest(): void {
    this.requestApplePayPaymentRequestQuery$.get().pipe(
      ...
      error: (): GlobalMessageActions.AddMessage => this.showPaymentMethodFailMessage('paymentForm.applePay.merchantValidationFailed'),
    });
  }
  ...
  public onShippingMethodSelected(event: { shippingMethod: ApplePayShippingMethod }): void {
    this.onShippingMethodSelected$(event.shippingMethod).get().pipe(
      ...
      error: (): GlobalMessageActions.AddMessage => this.showPaymentMethodFailMessage('paymentForm.applePay.setDeliveryMethodFailed')
    });
  }
  ...
  public onShippingContactSelected(event: { shippingContact: ApplePayPaymentContact }): void {
    this.onShippingContactSelectedCommand$.execute(event.shippingContact).pipe(
      ...
      error: (): GlobalMessageActions.AddMessage => this.showPaymentMethodFailMessage('paymentForm.applePay.setDeliveryAddressFailed')
    });
  }
  ...
  public onPaymentAuthorized(event: { payment: any }): void {
    this.onPaymentAuthorizedCommand$.execute(event.payment).pipe(
      ...
      error: (): GlobalMessageActions.AddMessage => this.showPaymentMethodFailMessage('paymentForm.applePay.authorisationFailed')
    });
  }
   ...
   * @returns {GlobalMessageActions.AddMessage} - The action to add the message.
   * @since 2211.31.1
   */
  public showPaymentMethodFailMessage(message: string): GlobalMessageActions.AddMessage {
