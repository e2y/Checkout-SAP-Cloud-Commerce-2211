...
import { Command, CommandService, CommandStrategy, EventService, GlobalMessageService, GlobalMessageType, LoggerService, UserIdService } from '@spartacus/core';
...
import { CheckoutComOrderResult, CheckoutComRedirect } from '../../interfaces';
...
export class CheckoutComOrderService extends OrderService implements CheckoutComOrderFacade {
  ...
  public authorizeOrder(sessionId: string): Observable<boolean> {
    return this.authorizeOrderCommand$.execute(sessionId).pipe(
      tap({
        error: (error: unknown): void => {
          this.globalMessageService.add({ key: 'checkoutReview.flowPopup.error.authorizeOrder' }, GlobalMessageType.MSG_TYPE_ERROR);
          this.logger.error('CheckoutComOrderService AuthorizeOrder error', { error });
          this.checkoutComOrderPlaced$.next(
            {
              order: undefined,
              successful: false,
      ...
      switchMap((): Observable<boolean> =>
        this.getOrderDetails().pipe(
          filter((order: Order): order is Order => order != null),
          distinctUntilChanged((a: Order, b: Order): boolean => a?.code === b?.code),
          take(1),
          map((): boolean => true)
        )
      ),
