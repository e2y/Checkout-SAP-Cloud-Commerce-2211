...
import { EventService, GlobalMessageService, GlobalMessageType, LoggerService, OCC_USER_ID_CURRENT, UserIdService } from '@spartacus/core';
import { OrderConnector } from '@spartacus/order/core';
import { Order, OrderPlacedEvent } from '@spartacus/order/root';
import { EMPTY, of, Subject, take, throwError } from 'rxjs';
import { CheckoutComOrderConnector } from '../../connectors/checkout-com-order/checkout-com-order.connector';
...
describe('CheckoutComCheckoutService', () => {
  ...
  describe('authorizeOrder', () => {
    ...
    it('should log error when authorization fails', (done) => {
      const sessionId = 'session123';
      ...
      service.authorizeOrder(sessionId).subscribe({
        next: () => {
          expect(service['logger'].error).toHaveBeenCalledWith(
            'CheckoutComOrderService AuthorizeOrder error',
            { error }
          );
          done();
        }
    ...
    it('should add global error message when authorization fails', (done) => {
      const sessionId = 'session123';
      const error = new Error('Authorization failed');
      spyOn(logger, 'error');
      ...
      service.authorizeOrder(sessionId).subscribe({
        next: (result) => {
          expect(globalMessageService.add).toHaveBeenCalledWith(
            { key: 'checkoutReview.flowPopup.error.authorizeOrder' },
            GlobalMessageType.MSG_TYPE_ERROR
          );
          expect(logger.error).toHaveBeenCalledWith('CheckoutComOrderService AuthorizeOrder error', { error });
          expect(result).toBeFalse();
          done();
        }
      });
    ...
    it('should update checkoutComOrderPlaced$ with error state when authorization fails', (done) => {
      const sessionId = 'session123';
      const error = new Error('Authorization failed');
      spyOn(logger, 'error');
      ...
      service.authorizeOrder(sessionId).subscribe({
        next: () => {
          const orderPlacedValue = service.getCheckoutComOrderPlacedValue();
          expect(orderPlacedValue).toEqual({
            order: undefined,
            successful: false,
            redirect: null,
            httpError: error
    ...
    it('should dispatch CheckoutComOrderPlacedEvent with error state when authorization fails', (done) => {
      const sessionId = 'session123';
      const error = new Error('Authorization failed');
      spyOn(logger, 'error');
