...
import { CheckoutComApmConnector } from '../../connectors/checkout-com-apm/checkout-com-apm.connector';
import { CheckoutComFlowConnector } from '../../connectors/checkout-com-flow/checkout-com-flow.connector';
import { CheckoutComPaymentConnector } from '../../connectors/checkout-com-payment/checkout-com-payment.connector';
import { CheckoutComConnector } from '../../connectors/checkout-com/checkout-com.connector';
import { CheckoutComBillingAddressUpdatedEvent } from '../../events/billing-address.events';
import { CheckoutComClearPaymentSessionEvent } from '../../events/checkout-com-flow.events';
import { CheckoutComOrderPlacedEvent } from '../../events/checkout-order.events';
import { CheckoutComCustomerConfiguration, CheckoutComFlowUIConfigurationData, CheckoutComPaymentSession, FlowEnabledDataResponseDTO } from '../../interfaces';
...
describe('CheckoutComFlowService', () => {
  ...
  beforeEach(() => {
    ...
    checkoutComConnector.getIsABC.and.returnValue(of(false));
    ...
    baseSiteService = jasmine.createSpyObj('BaseSiteService', ['getActive']);
    baseSiteService.getActive.and.returnValue(of('electronics-spa'));
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: baseSiteService
        }
      ]
    });
    service = TestBed.inject(CheckoutComFlowService);
    baseSiteService = TestBed.inject(BaseSiteService) as jasmine.SpyObj<BaseSiteService>;
  ...
  describe('getIsFlowEnabled', () => {
    it('should emit true when isFlowEnabled$ is set to true', (done) => {
      spyOn(service, 'requestIsFlowEnabled').and.returnValue(of({
        error: false,
        loading: false,
        data: { enabled: true }
      }));
    ...
    it('should emit false value if isFlowEnabled$ is not set', (done) => {
  ...
  describe('getFlowUIConfiguration', () => {
    it('should return the current flow appearance configuration when it is set', (done) => {
      ...
      service.getFlowUIConfiguration().subscribe({
        next: (flowAppearanceConfiguration) => {
          expect(flowAppearanceConfiguration).toEqual(mockConfiguration);
          done();
        }
      });
    });

    it('should return an empty object when no flow appearance configuration is set', (done) => {
      service['flowAppearanceConfiguration'] = {};

      service.getFlowUIConfiguration().subscribe({
        next: (flowAppearanceConfiguration) => {
          expect(flowAppearanceConfiguration).toEqual({});
          done();
        }
      });
    });
  ...
  describe('initializeFlow', () => {
    it('should request flow enabled state and public key when flow is enabled', () => {
      ...
      spyOn<any>(service, 'requestPublicKey').and.returnValue(of('public-key'));
      spyOn<any>(service, 'requestFlowUIConfiguration').and.returnValue(of({}));

      service.initializeFlow().subscribe((result) => {
  ...
  describe('requestPublicKey', () => {
    it('should call setFlowPublicKey with the retrieved public key', () => {
      ...
      (service as any).requestPublicKey().subscribe({
        next: (key) => {
          expect(key).toEqual('test-public-key');
        }
      });
  ...
  describe('loadWebComponents', () => {
    it('should create the correct options and call loadCheckoutWebComponents', (done) => {
      ...
      spyOn<any>(service, 'getEnvironment').and.returnValue(expectedEnvironment);
  ...
  describe('resetOnPaymentSessionEvents', () => {
    it('returns the correct list of events for resetting the payment session', () => {
      ...
      expect(events).toEqual([
        ...
        CheckoutComClearPaymentSessionEvent
      ]);
    });
  });
