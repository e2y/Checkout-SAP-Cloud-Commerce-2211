...
} from '../../events/checkout-com-flow.events';
import { CheckoutComOrderPlacedEvent } from '../../events/checkout-order.events';
import { CheckoutComApmFacade } from '../../facades/checkout-com-apm.facade';
import { CheckoutComFlowFacade } from '../../facades/checkout-com-flow.facade';
import { CheckoutComFlowComponentInterface, CheckoutComFlowUIConfigurationData, CheckoutComPaymentSession, FlowEnabledDataResponseDTO } from '../../interfaces';
import { CheckoutComPaymentService } from '../payment/checkout-com-payment.service';
...
export class CheckoutComFlowService extends CheckoutComPaymentService implements CheckoutComFlowFacade {
  ...
  protected generatePaymentSession$: Query<CheckoutComPaymentSession> =
    this.queryService.create<CheckoutComPaymentSession>(
        ...
        ),
      {
        ...
        resetOn: this.resetOnPaymentSessionEvents()
      }
    );
  ...
  public getIsFlowEnabled(): Observable<boolean> {
    return this.requestIsFlowEnabled().pipe(
      ...
      map((state: QueryState<{ enabled: boolean }>): boolean => {
        const enabled: boolean = state.data?.enabled ?? false;
        this.setIsFlowEnabled(enabled);
        return enabled;
      }),
   ...
   * @since 2211.32.1
   */
  public getFlowUIConfiguration(): Observable<CheckoutComFlowUIConfigurationData> {
    return of(this.flowAppearanceConfiguration);
  }
   ...
   * @since 2211.32.1
   */
  public initializeFlow(): Observable<boolean> {
    return this.requestIsFlowEnabled().pipe(
      ...
      switchMap((enabled: boolean): Observable<boolean> => {
        if (!enabled) {
          return of(false);
        }
        return this.requestPublicKey().pipe(
  ...
  public clearPaymentSession(): void {
    this.flowPaymentSession$.next(null);
    this.eventService.dispatch(null, CheckoutComClearPaymentSessionEvent);
  }
   ...
   * @since 2211.32.1
   */
  protected requestPublicKey(): Observable<string> {
    return this.getOccMerchantKeyFromState().pipe(
      filter((key: string): boolean => Boolean(key)),
      distinctUntilChanged(),
      tap((publicKey: string): void => {
