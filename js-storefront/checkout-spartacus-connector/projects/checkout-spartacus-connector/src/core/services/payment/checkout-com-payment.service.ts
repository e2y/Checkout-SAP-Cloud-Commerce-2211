...
export class CheckoutComPaymentService extends CheckoutPaymentService implements CheckoutComPaymentFacade {
  ...
  protected setPaymentAddressCommand: Command<Address, Address> = this.commandService.create(
    (address: Address): Observable<Address> => this.checkoutPreconditions().pipe(
      switchMap(([userId, cartId]: [string, string]): Observable<Address> =>
        this.checkoutPaymentConnector.setPaymentAddress(userId, cartId, address).pipe(
  ...
  protected requestOccMerchantKeyQuery$: Query<string, []> = this.queryService.create(
    (): Observable<string> => this.userIdService.getUserId().pipe(
      switchMap((userId: string): Observable<string> => this.checkoutComConnector.getMerchantKey(userId)
        .pipe(
          tap((occMerchantKey: string): void => {
            this.occMerchantKey$.next(occMerchantKey);
          }),
        )
      )
    ),
   ...
   * @param {CheckoutComConnector} checkoutComConnector - Connector for Checkout.com.
   * @param {CheckoutComApmConnector} checkoutApmConnector - Connector for Checkout.com APM.
   *
   * @since 2211.31.1
   */
  constructor(
    ...
    protected checkoutComConnector: CheckoutComConnector,
    protected checkoutApmConnector: CheckoutComApmConnector,
  ) {
    super(
      activeCartFacade,
      userIdService,
  ...
  public getOccMerchantKeyFromState(): Observable<string> {
    return this.requestOccMerchantKey().pipe(
   ...
   * @since 4.2.7
   */
  public requestOccMerchantKey(): Observable<string> {
    return this.requestOccMerchantKeyQuery$.get();
  }
