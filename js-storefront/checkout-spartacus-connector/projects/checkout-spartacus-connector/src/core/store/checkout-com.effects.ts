...
import { GooglePayMerchantConfiguration, PaymentDataRequestUpdate, PlaceOrderResponse } from '../model/GooglePay';
import { ApplePayAuthorization, ApplePayPaymentRequest, ApplePayShippingContactUpdate, ApplePayShippingMethodUpdate } from '../model/ApplePay';
...
export class CheckoutComEffects {
  ...
  @Effect()
  getIsABC$: Observable<| CheckoutComActions.SetOccIsABCSuccess
    | GlobalMessageActions.AddMessage
    | CheckoutComActions.SetOccIsABCFail> = this.actions$.pipe(
    ofType(CheckoutComActions.SET_OCC_IS_ABC),
    map((action: any) => action.payload),
    mergeMap(payload => this.checkoutComAdapter.getIsABC(payload.userId)
      .pipe(
        switchMap(data => [new CheckoutComActions.SetOccIsABCSuccess(data)]),
        catchError(error => [
          new CheckoutComActions.SetOccIsABCFail(error),
          new GlobalMessageActions.AddMessage({
            type: GlobalMessageType.MSG_TYPE_ERROR,
            text: {
  ...
  @Effect()
  setKlarnaInitParams: Observable<CheckoutComActions.SetKlarnaInitParamsSuccess
    | CheckoutComActions.SetKlarnaInitParamsFail> = this.actions$.pipe(
  ...
  @Effect()
  checkoutUpdatePaymentDetails$: Observable<
    | CheckoutComActions.CheckoutUpdatePaymentDetailsSuccess
    | GlobalMessageActions.AddMessage
    | CheckoutComActions.CheckoutUpdatePaymentDetailsFail
    | UserActions.LoadUserPaymentMethods
  > = this.actions$.pipe(
    ofType(CheckoutComActions.CHECKOUT_UPDATE_PAYMENT_DETAILS),
    map((action: any) => action.payload),
    mergeMap(({
        userId,
        cartId,
        paymentDetails
      }) => this.checkoutComAdapter.updatePaymentDetails(userId, cartId, paymentDetails)
        .pipe(
          switchMap(data => [
            new CheckoutComActions.CheckoutUpdatePaymentDetailsSuccess(data),
            new UserActions.LoadUserPaymentMethods(userId)
          ]),
          catchError(error => [
            new CheckoutComActions.CheckoutUpdatePaymentDetailsFail(error),
            new GlobalMessageActions.AddMessage({
              type: GlobalMessageType.MSG_TYPE_ERROR,
              text: {
                key: 'paymentForm.merchantKeyFailed'
  ...
  @Effect()
  setAchLinkToken: Observable<CheckoutComActions.SetAchLinkTokenSuccess
    | CheckoutComActions.SetAchLinkTokenFail> = this.actions$.pipe(
    ofType(CheckoutComActions.SET_ACH_LINK_TOKEN),
    map((action: any) => action.payload),
    mergeMap((payload) => this.checkoutComAdapter.getAchLinkToken(payload.userId, payload.cartId)
      .pipe(
        switchMap((params) => [new CheckoutComActions.SetAchLinkTokenSuccess(params)]),
        catchError(error => [new CheckoutComActions.SetAchLinkTokenFail(error)])
      )
    )
  );

  @Effect()
  SetAchOrderSuccess: Observable<CheckoutComActions.SetAchOrderSuccessSuccess
    | CheckoutComActions.SetAchOrderSuccessFail
    | CheckoutActions.PlaceOrderSuccess
    | CartActions.RemoveCart
    | CheckoutComActions.PlaceOrderSuccess
    | GlobalMessageActions.AddMessage> = this.actions$.pipe(
    ofType(CheckoutComActions.SET_ACH_SUCCESS_ORDER),
    map((action: any) => action.payload),
    mergeMap((payload) => this.checkoutComAdapter
      .getAchOrderSuccess(payload.userId, payload.cartId, payload.publicToken, payload.metadata, payload.customerConsents)
      .pipe(
        switchMap((params) => [
          new CheckoutComActions.SetAchOrderSuccessSuccess(params),
          new CheckoutActions.PlaceOrderSuccess(params),
          new CartActions.RemoveCart({ cartId: payload.cartId }),
          new CheckoutComActions.PlaceOrderSuccess(params),
        ]),
        catchError((error: any) => [
          new CheckoutComActions.SetAchOrderSuccessFail(normalizeHttpError(error)),
          createPaymentFailAction('checkoutReview.initialPaymentRequestFailed'),
        ])
      )
    )
  );
