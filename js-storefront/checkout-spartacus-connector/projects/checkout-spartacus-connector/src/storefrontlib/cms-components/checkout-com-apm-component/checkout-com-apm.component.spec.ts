...
import { Address, Currency, CurrencyService, GlobalMessageService, GlobalMessageType, MockTranslatePipe, QueryState, WindowRef } from '@spartacus/core';
...
describe('CheckoutComApmComponent', () => {
  ...
  describe('isBillingAddressEditModeEnabled', () => {
    it('should return true when billing address edit mode is enabled', () => {
      spyOn(checkoutBillingAddressFormService, 'isEditModeEnabled').and.returnValue(of(true));

      component.isBillingAddressEditModeEnabled().subscribe((result) => {
        expect(result).toBeTrue();
      });
    });

    it('should return false when billing address edit mode is disabled', () => {
