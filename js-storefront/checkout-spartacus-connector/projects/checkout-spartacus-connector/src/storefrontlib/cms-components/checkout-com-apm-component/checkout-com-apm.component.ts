...
export class CheckoutComApmComponent implements OnInit {
  ...
  billingAddressForm: UntypedFormGroup = this.checkoutBillingAddressFormService.getBillingAddressForm();
  public sameAsShippingAddress$: BehaviorSubject<boolean> = this.checkoutBillingAddressFormService.getSameAsDeliveryAddress();
  ...
  selectApmPaymentDetails(): void {
    ...
    if (!this.sameAsShippingAddress$.value) {
      if (this.checkoutBillingAddressFormService.isBillingAddressFormValid()) {
        billingAddress = this.checkoutBillingAddressFormService.billingAddress$.value;
      } else {
   ...
   * @since 2211.32.1
   */
  isBillingAddressEditModeEnabled(): Observable<boolean> {
    return this.checkoutBillingAddressFormService.isEditModeEnabled();
  }
