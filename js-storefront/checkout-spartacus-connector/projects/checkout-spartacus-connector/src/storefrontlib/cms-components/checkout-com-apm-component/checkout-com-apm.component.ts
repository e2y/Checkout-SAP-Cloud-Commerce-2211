...
import { BehaviorSubject, combineLatest, Observable, ObservableInput, Subject } from 'rxjs';
...
import { filter, takeUntil, take, switchMap, first, map, finalize, tap } from 'rxjs/operators';
...
export class CheckoutComApmComponent implements OnInit, OnDestroy {
    ...
    .pipe(
      filter(([apms, selectedApm]) =>
        ...
        !!selectedApm && (
          ...
          && selectedApm.code !== PaymentType.ACH
        )),
      takeUntil(this.drop),
  ...
  /**
   * If Ach is available, we request a token and wait for response.
   * When the response arrives, we show the Ach button
   */
  protected initializeAch(): void {
    this.ach$ = combineLatest([
      this.checkoutComApmService.getApmByComponent('achComponent', PaymentType.Ach)
    ]).pipe(
      filter(([component]) => {
        return !!component;
      }),
